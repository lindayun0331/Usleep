...
export default {
    ...
    computed: {
        yearDateRange() {
            const start = this.currentYearStart;
            const end = this.currentYearEnd;
            const startYear = start.getFullYear();
            const endYear = end.getFullYear();
            
            if (startYear === endYear) {
                return `${startYear}年1月1日--12月31日`;
            } else {
                return `${startYear}年1月1日--${endYear}年12月31日`;
            }
        },
        // 计算年均睡眠时长
        averageSleepTime() {
            if (this.yearSleepData.deepSleep && this.yearSleepData.deepSleep.length > 0) {
                // 计算深睡平均时长
                const deepSleepTotal = this.yearSleepData.deepSleep.reduce((sum, hours) => sum + hours, 0);
                const deepSleepAverage = deepSleepTotal / this.yearSleepData.deepSleep.length;
                
                // 计算浅睡平均时长
                const lightSleepTotal = this.yearSleepData.lightSleep.reduce((sum, hours) => sum + hours, 0);
                const lightSleepAverage = lightSleepTotal / this.yearSleepData.lightSleep.length;
    ...
    methods: {
        ...
        async getYearReportData() {
            ...
            try {
                ...
                this.yearReportData = res.data;
                console.log("年报告数据获取成功:", this.yearReportData);
                
                //统计图：时间轴
                this.timeList = this.yearReportData.timeList;
                
                // 统计图数据：将数据转换为小时并赋值给数组
                const data = this.yearReportData;
                if (data.deep_value && Array.isArray(data.deep_value)) {
                    this.yearSleepData.deepSleep = data.deep_value.map(minutes => minutes / 60);
                }
                if (data.light_value && Array.isArray(data.light_value)) {
                    this.yearSleepData.lightSleep = data.light_value.map(minutes => minutes / 60);
                }
                if (data.wake_value && Array.isArray(data.wake_value)) {
                    this.yearSleepData.awake = data.wake_value.map(minutes => minutes / 60);
                }  
        ...
        initCurrentYear() {
            ...
            this.currentYearStart = yearStart;
            this.currentYearEnd = yearEnd;
        },
        
        goToPreviousYear() {
            const newStart = new Date(this.currentYearStart);
            const newEnd = new Date(this.currentYearEnd);
            
            newStart.setFullYear(newStart.getFullYear() - 1);
            newEnd.setFullYear(newEnd.getFullYear() - 1);
            
            this.currentYearStart = newStart;
            this.currentYearEnd = newEnd;
            this.getYearReportData();
        },
    ...
    <view class="week-report">
        ...
        <view class="sleep-stats">
            ...
            <view class="chart-container">
                <!-- Y轴标签 -->
                <view class="y-axis">
                    <text class="y-label">100H</text>
                    <text class="y-label">75H</text>
                    <text class="y-label">50H</text>
                    <text class="y-label">25H</text>
                ...
                <view class="chart-main">
                    ...
                    <view class="chart-bars">
                        <view 
                            v-for="(deep, index) in yearSleepData.deepSleep" 
                            :key="index" 
                            class="chart-bar"
                        >
                            <!-- 清醒 -->
                            <view 
                                ...
                                :style="{ height: (yearSleepData.awake[index] * 100 / 100) + '%' }"
                            ></view>
                            <!-- 浅睡 -->
                            <view 
                                class="bar-segment light-sleep"
                                :style="{ height: (yearSleepData.lightSleep[index] * 100 / 100) + '%' }"
                            ></view>
                            <!-- 深睡 -->
                            <view 
                                class="bar-segment deep-sleep"
                                :style="{ height: (deep * 100 / 100) + '%' }"
                            ></view>
                        </view>
                    </view>
                    <!-- X轴标签 -->
